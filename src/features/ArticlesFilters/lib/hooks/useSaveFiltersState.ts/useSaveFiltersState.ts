...
export const useSaveFilterState = (callback: (filters: ArticlesFilters) => void) => {
    ...
    const { sort: currentSort, order: currentOrder, search: currentSearch, type: currentType } = currentFilters;

    const saveFilters = useCallback(
        (filters: ArticlesFilters) => {
            const { order, sort, search, type } = filters;
            const valuesChanged =
                order !== currentOrder || sort !== currentSort || currentSearch !== search || currentType !== type;
        ...
        [dispatch, callback, currentOrder, currentSearch, currentSort, currentType],
    );
    ...
    useEffect(() => {
        ...
        const filterFromParams =
            Array.from(searchParams.entries()).length > 0 ? Object.fromEntries(searchParams.entries()) : null;

        if (filterFromParams) {
            saveFilters(filterFromParams as unknown as ArticlesFilters);
            localStorage.setItem(ARTICLES_FILTERS, JSON.stringify(filterFromParams));
    ...
    }, [currentFilters, callback, dispatch, searchParams, saveFilters, setSearchParams]);
};
