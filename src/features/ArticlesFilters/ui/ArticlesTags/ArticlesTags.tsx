...
export const ArticlesTags: FC<ArticlesTagsProps> = memo((props) => {
    ...
    const tags: TagItem<ArticleType>[] = useMemo(
        () =>
            Object.entries(articleTypesTranslations).map((val) => ({
                value: val[0] as ArticleType,
                content: t(val[1]),
            })),
        [t],
    );

    return <Tags items={tags} value={value} className={classNames('', {}, [className])} onChange={onChange} />;
});
