...
const LoginForm: FC<LoginFormProps> = ({ className, onSuccess }) => {
    ...
    const handleUsernameChange = useCallback((name: string) => dispatch(loginActions.setUsername(name)), [dispatch]);

    const handlePasswordChange = useCallback((ps: string) => dispatch(loginActions.setPassword(ps)), [dispatch]);
